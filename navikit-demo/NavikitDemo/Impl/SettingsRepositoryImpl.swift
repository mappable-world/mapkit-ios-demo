...
extension MMKDrivingVehicleType: Codable {}
extension MMKAnnotationLanguage: Codable {}

class SettingsRepositoryImpl: SettingsRepository {
    var styleMode: CurrentValueSubject<StyleMode, Never> = setting("styleMode", .system)

    // Vehicle options
    var vehicleType: CurrentValueSubject<MMKDrivingVehicleType, Never> = setting("vehicleType", .default)
    ...
    var roadEventsOnRoute: [MMKRoadEventsEventTag: CurrentValueSubject<Bool, Never>] = {
        Dictionary(
            uniqueKeysWithValues: MMKRoadEventsEventTag.allCases.map { tag in
                (tag, setting("roadEventsOnRoute_\(tag.rawValue)", true))
            }
        )
    }()
    var roadEventsOnLayer: [MMKRoadEventsEventTag: CurrentValueSubject<Bool, Never>] = {
        Dictionary(
            uniqueKeysWithValues: MMKRoadEventsEventTag.allCases.map { tag in
    ...
    var annotationLanguage: CurrentValueSubject<MMKAnnotationLanguage, Never> = setting("annotationLanguage", .russian)
