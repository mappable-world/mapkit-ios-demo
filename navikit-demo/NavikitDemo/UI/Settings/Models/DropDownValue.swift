...
struct DropDownValue {
    ...
    init(settingsRepository: SettingsRepository, type: DropDownType) {
        ...
        case .vehicleType:
            self = Self(
                options: MMKDrivingVehicleType.allCases.map { $0.description.capitalized },
                selected: settingsRepository.vehicleType.map { $0.rawValue }.eraseToAnyPublisher()
            ) { index in
                settingsRepository.vehicleType.send(MMKDrivingVehicleType.allCases[index])
            }
        ...
        case .annotationsLanguage:
            self = Self(
                options: MMKAnnotationLanguage.allCases.map { $0.description.capitalized },
                selected: settingsRepository.annotationLanguage.map { $0.rawValue }.eraseToAnyPublisher()
            ) { index in
                settingsRepository.annotationLanguage.send(MMKAnnotationLanguage.allCases[index])
            }
