...
class MapObjectsViewController: BaseMapViewController {
    ...
    override func viewDidLoad() {
        ...
        mapView.mapWindow.map.move(
            with: MMKCameraPosition.init(target: Const.targetLocation, zoom: 15, azimuth: 0, tilt: 0))
    }
    ...
    func createMapObjects() {
        ...
        let coloredPolyline = mapObjects.addPolyline(with: MMKPolyline(points: Const.coloredPolylinePoints))
    ...
    func createTappableCircle() {
        let mapObjects = mapView.mapWindow.map.mapObjects;
        let circle = mapObjects.addCircle(with: MMKCircle(center: Const.circleCenter, radius: 100))
    ...
    func createAnimatedPlacemark() {
        ...
        animation.setIconWithImage(animatedImageProvider, style: MMKIconStyle()) {
            animation.play()
        }
    }
}
