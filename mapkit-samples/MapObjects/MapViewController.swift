...
class MapViewController: UIViewController {
    ...
    private func move(_ map: MMKMap, to cameraPosition: MMKCameraPosition = GeometryProvider.startPosition) {
        map.move(with: cameraPosition, animation: MMKAnimation(type: .smooth, duration: 1.0))
    }

    /// Sets the map to specified geometry
    private func move(_ map: MMKMap, to geometry: MMKGeometry) {
    ...
    private func addPlacemarks() {
        ...
        if let animatedImageProvider = MRTAnimatedImageProviderFactory.fromFile(
            Bundle.main.path(forResource: "animation", ofType: "png")
        ) as? MRTAnimatedImageProvider {
    ...
    private func updateMapFocus() {
        ...
        mapView.mapWindow.focusRect = MMKScreenRect(
            topLeft: MMKScreenPoint(x: Float(Layout.horizontalMargin) * scale, y: Float(Layout.verticalMargin) * scale),
    ...
    private var pinsCollection: MMKMapObjectCollection!
    private var clusterizedCollection: MMKClusterizedPlacemarkCollection!
    private var polygonMapObject: MMKPolygonMapObject!
    private var polylineMapObject: MMKPolylineMapObject!
    ...
    private func focusPolygonButtonTapHandler() {
        let geometry = MMKGeometry(polygon: polygonMapObject.geometry)
        move(map, to: geometry)
    }

    @objc
    private func focusPolylineButtonTapHandler() {
        let geometry = MMKGeometry(polyline: polylineMapObject.geometry)
        move(map, to: geometry)
    }
