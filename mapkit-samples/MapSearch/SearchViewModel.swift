...
class SearchViewModel {
    ...
    func setVisibleRegion(with region: MMKVisibleRegion) {
    ...
    func startSearch(with searchText: String? = nil) {
        ...
        submitSearch(with: text, geometry: MMKVisibleRegionUtils.toPolygon(with: visibleRegion))
    }
    ...
    private func setupSearchSubscription() {
        $debouncedVisibleRegion
            ...
            .sink { [weak self] visibleRegion in
                ...
                self.searchSession?.setSearchAreaWithArea(MMKVisibleRegionUtils.toPolygon(with: visibleRegion))
    ...
    private func submitSearch(with query: String, geometry: MMKGeometry) {
        searchSession?.cancel()
    ...
    private func submitSuggest(with query: String, boundingBox: MMKBoundingBox, options: MMKSuggestOptions) {
    ...
    private func handleSuggestSessionResponse(response: MMKSuggestResponse?, error: Error?) {
        if let error = error {
    ...
    @Published private var visibleRegion: MMKVisibleRegion?
    @Published private var debouncedVisibleRegion: MMKVisibleRegion?
